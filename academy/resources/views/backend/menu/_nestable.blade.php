    ...
    @foreach($data as $d)
        <li class="dd-item dd3-item" data-id="{{ $d->id }}">
            ...
            <div class="dd3-content">
            	...
            	@if(!$d->child->count())
            	<a href="javascript:void(0)" class="pull-right text-danger delete" title="Xóa" data-url="{{ url('admin/menu/' . $d->id) }}"><i class="fa fa-times"></i></a>
            	@endif
            ...
            @if($d->child->count())
                @include('backend.menu._nestable', ['data' => $d->child])
            @endif
        </li>
    @endforeach
</ol>